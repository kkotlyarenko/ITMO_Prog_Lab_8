...
public class AddWindow {
    ...
    public static void createAndShowGUI(ResourceBundle bundle) {
        JFrame frame = new JFrame(bundle.getString("window.title.add"));
        ...
        addButton.addActionListener(e -> {
            ...
            if (title.isEmpty() || x1.isEmpty() || y1.isEmpty() || x2.isEmpty() || y2.isEmpty() || z2.isEmpty() || x3.isEmpty() || y3.isEmpty() || name3.isEmpty() || distance.isEmpty()) {
                ...
                try {
                    ...
                    if (distanceValue <= 0) {
                        ...
                        Request addroute = new Request("add", new UserCollection(), "", main.socket.Client.username, main.socket.Client.password);
                        addroute.getCollection().add(newroute);

                        Response resp = main.socket.Client.sendAndGet(addroute);
                        System.out.println(resp.getText());

                        JOptionPane.showMessageDialog(frame, bundle.getString("success.add"), bundle.getString("success.title"), JOptionPane.INFORMATION_MESSAGE);
