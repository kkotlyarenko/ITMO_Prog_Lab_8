...
public class AddIfMax extends Command {
    public AddIfMax() {
        super("add_if_max", "{element} - add a new element to the collection if its value exceeds the maximum value of the collection element.");
    }
    ...
    public Response execute(Request request) {
        ...
        if (request.getCollection().getFirst().compareTo(CollectionManager.getInstance().getMax()) > 0) {
